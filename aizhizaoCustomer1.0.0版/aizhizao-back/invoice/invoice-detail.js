...
Module.define("system.invoice", function(page, $) {
	page.ready = function() {
		...
		$("#Subbunnot").bind("click", signingInvoice);
	}
	
	function signingInvoice() {
		$.ajax({
			type: "POST",
			url: ulrTo+"/azz/api/client/invoice/signingInvoice",
			cache: false, //禁用缓存
			data: {
	...
	function getClientInvoiceApplyDetail() {
		$.ajax({
			type: "POST",
			url: ulrTo+"/azz/api/client/invoice/getClientInvoiceApplyDetail",
			cache: false, //禁用缓存
			data: {
				'clientOrderCode' : clientOrderCode
			},
			...
			success: function(data) {
				if (data.code == 0) {
					...
					//开票信息
					var invoiceDelivery = data.invoiceDelivery;
					var trr = "";
					for(var i = 0;i < invoiceDelivery.length; i++){
						var applyAmount = invoiceDelivery[i].applyAmount;
