...
Module.define("system.personal", function(page, $) {
	page.ready = function() {
		...
		$('#userCode').html(userInfo.userCode);
		$('#userName').html(userInfo.userName);
		$('#phoneNumber').html(userInfo.phoneNumber);
		$("input[name='Phone']").val(userInfo.phoneNumber);
		$('#postName').html(userInfo.postName);
		$('#deptCode').html(userInfo.deptCode);
		$('#deptName').html(userInfo.deptName);
		$('#roleName').html(userInfo.roleName);
		$('#roleCode').html(userInfo.roleCode);
	...
	function userNameEid() {
		...
		$.ajax({
			type: "POST",
			url: ulrTo+"/azz/api/user/editPersonalInfo",
			cache: false, //禁用缓存
			data: {
				'editType': 1,
			...
			success: function(data) {
				if (data.code == 0) {
					var arr = userInfo;
					arr.userName = $("input[name='Name']").val();
	...
	function checkEditVerificationCode() {
		...
		$.ajax({
			type: "POST",
			url: ulrTo+"/azz/api/user/checkEditVerificationCode",
			cache: false, //禁用缓存
			data: {
    ...
    page.ronglian = function() {
	   	...
	   	$.ajax({
	   		type: "POST",
	   		url: ulrTo+"/azz/api/user/sendEditVerificationCode",
	   		data: param,
   ...
   var sessiontime3 = 60;
    ...
    page.ronglian3 = function() {
	   	...
	   	$.ajax({
	   		type: "POST",
	   		url: ulrTo+"/azz/api/user/sendEditEmailVerificationCode",
	   		data: param,
	   		complete: function(XMLHttpRequest, textStatus) {
