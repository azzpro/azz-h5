Module.define("system.parameter", function(page, $) {
		...
		dataTable = $('#table').DataTable({
			...
			"ajax": function (data, callback, settings) {
				...
				//当前页码
				 $.ajax({
				 	type: "POST",   
				 	url: ulrTo + "/azz/api/merchant/combination/getCombinationInfoList",
				 	cache: false, //禁用缓存   
				 	data: param, //传入组装的参数   
				 	dataType: "json", 
				...
				},
				{
					...
					"render" : function (data, type, row, meta) {
						if (row) {
		            		...
		            		html += '<a href="#!parameter/parameter-edit.html?combinationCode={0}">编辑</a>'.format(row.combinationCode);
	            			html += '&nbsp;&nbsp;<a class="text-nowrap" href="javascript:;" onclick="system.parameter.editUserStatus(\'' + row.combinationCode + "','"+ statustoo + '\');">'+ statustoo +'</a>';
	...
	//启用禁用
	page.editUserStatus = function(combinationCode,statustoo) {
		if(statustoo == '上架') {
			$.ajax({
				type: "POST",
				url: ulrTo+"/azz/api/merchant/combination/putOnOrPutOffOrDelCombination",
				cache: false, //禁用缓存    
				dataType: "json", 
				data: {
	...
	//删除
	page.delDeptInfo = function(combinationCode,combinationName) {
		$('#bmName').html(combinationName);
		$('#myModal112').modal('show');
		$('#deletebunnot').attr("onclick", "system.parameter.delDeptInfotoo(\'" + combinationCode + "\');")
	}
	page.delDeptInfotoo = function(combinationCode) {
		$.ajax({
			type: "POST",
			url: ulrTo+"/azz/api/merchant/combination/putOnOrPutOffOrDelCombination",
			cache: false, //禁用缓存    
