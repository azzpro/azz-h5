Module.define("system.brand", function(page, $) {
				...
				},
				{
					...
					"render" : function (data, type, row, meta) {
						if (row) {
		            		...
		            		html += '<a href="javascript:;" onclick="system.brand.editDeptInfo(\'' + row.brandCode + '\');">编辑</a>';
		            		html += '&nbsp;&nbsp;<a href="javascript:;" onclick="system.brand.delDeptInfo(\'' + row.brandCode + "','"+ row.brandName + '\');">删除</a>';
		            		html += '</div>';
		            		html += '</div>';
			         		return html;
						}
	...
	function addGoodsBrand() {
		...
		fm.append('brandEnglishName', $("input[name='brandEnglishName']").val());
		fm.append('brandDescription', $("textarea[name='brandDescription']").val());
		fm.append('goodsBrandPicFile', file1);
		$.ajax({
	        type :'POST',
	        url : ulrTo+'/azz/api/merchant/goodsBrand/addGoodsBrand',
	        cache: false, //禁用缓存    
			dataType: "json",
	...
	page.editDeptInfo = function(brandCode) {
		$.ajax({
			type: "POST",
			url: ulrTo+"/azz/api/merchant/goodsBrand/getGoodsBrandInfo",
			cache: false, //禁用缓存    
			dataType: "json", 
			data: {
				'brandCode': brandCode
			...
			success: function(data) {
				if (data.code == 0) {
					...
					$("input[name='brandEnglishName2']").val(data.data.brandEnglishName);
		...
		$('#myModal2').modal('show');
		$('#editconfirm').attr("onclick", "system.brand.editDeptInfotoo(\'" + brandCode + "\');")
	}
	...
	page.editDeptInfotoo = function(brandCode) {
		...
		fm.append('brandEnglishName', $("input[name='brandEnglishName2']").val());
		fm.append('brandDescription', $("textarea[name='brandDescription2']").val());
		if(!file2){}else{fm.append('goodsBrandPicFile', file2);}
		fm.append('isChangeGoodsBrandPic', isChangeGoodsBrandPic);
		
		$.ajax({
	        type :'POST',
	        url : ulrTo+'/azz/api/merchant/goodsBrand/editGoodsBrand',
	        cache: false, //禁用缓存    
			dataType: "json",
			contentType: false, //禁止设置请求类型
	        processData: false, //禁止jquery对DAta数据的处理,默认会处理
	...
	page.delDeptInfo = function(brandCode,brandName) {
		...
		$('#deletebunnot').attr("onclick", "system.brand.delDeptInfotoo(\'" + brandCode + "\');")
	}
	
	page.delDeptInfotoo = function(brandCode) {
		$.ajax({
			type: "POST",
			url: ulrTo+"/azz/api/merchant/goodsBrand/delGoodsBrand",
			cache: false, //禁用缓存    
			dataType: "json", 
			data: {
				'brandCode': brandCode
