...
Module.define("system.putforward", function(page, $) {
	page.ready = function() {
		getWithdrawDepositApplyDetail();
	}
	function getWithdrawDepositApplyDetail() {
		$.ajax({
			type: "POST",
			url: ulrTo+"/azz/api/merchant/finance/getWithdrawDepositApplyDetail",
			cache: false, //禁用缓存
			data: {
				'applyCode':applyCode,
			},
			...
			success: function(data) {
				if (data.code == 0) {
					...
					$('#remark').html(applyInfo.remark?applyInfo.remark:'-');
