Module.define("system.module", function(page, $) {
	...
	page.editUserStatus = function(moduleCode,statustoo) {
		if(statustoo == '上架') {
			$.ajax({
				type: "POST",
				url: ulrTo+"/azz/api/merchant/goodsModule/putOnOrPutOffOrDelGoodsModule",
				cache: false, //禁用缓存    
				dataType: "json", 
				data: {
	...
	//删除
	page.delDeptInfo = function(moduleCode,moduleName) {
		$('#bmName').html(moduleName);
		$('#myModal112').modal('show');
		$('#deletebunnot').attr("onclick", "system.module.delDeptInfotoo(\'" + moduleCode + "\');")
	}
	page.delDeptInfotoo = function(moduleCode) {
			$.ajax({
				type: "POST",
				url: ulrTo+"/azz/api/merchant/goodsModule/putOnOrPutOffOrDelGoodsModule",
				cache: false, //禁用缓存    
