Module.define("system.order", function(page, $) {
		...
		dataTable = $('#table').DataTable({
			...
			"ajax": function (data, callback, settings) {
				...
				//当前页码
				 $.ajax({
				 	type: "POST",   
				 	url: ulrTo + "/azz/api/merchant/order/getMerchantOrderList",
				 	cache: false, //禁用缓存   
				 	data: param, //传入组装的参数   
				 	dataType: "json", 
				...
				},
				{
					...
					"render" : function (data, type, row, meta) {
						if (row) {
		            		...
		            		html += '<a href="javascript:;" onclick="system.order.details(\'' + row.orderCode + "','"+ row.statusName + '\');">详情</a>';
		            		html += '</div>';
		            		html += '</div>';
			         		return html;
	...
	//详情
	page.details = function(orderCode,statusName) {
