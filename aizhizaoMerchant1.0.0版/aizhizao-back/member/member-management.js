Module.define("system.member", function(page, $) {
		...
		dataTable = $('#table').DataTable({
			...
			"ajax": function (data, callback, settings) {
				...
				//当前页码
				 $.ajax({
				 	type: "GET",   
				 	url: ulrTo + "/azz/api/merchant/getMerchantUserList",
				 	cache: false, //禁用缓存   
				 	data: param, //传入组装的参数   
				 	dataType: "json", 
			...
			"columns": [{
				...
				},
				{
					...
					"render" : function (data, type, row, meta) {
						...
						if(row.isMerchantRegister == 1){
							var htmlatt = ''
						}else{
							var htmlatt = '<a href="javascript:;" onclick="system.member.getUserInfo(\'' + row.merchantUserCode + '\');">编辑</a>&nbsp;&nbsp;<a href="javascript:;" onclick="system.member.editUserStatus(\'' + row.merchantUserCode + "','"+ statustoo + '\');">'+ statustoo +'</a>&nbsp;&nbsp;<a class="text-nowrap" href="javascript:;" onclick="system.member.delDeptInfo(\'' + row.merchantUserCode + "','"+ row.merchantUserName + '\');">删除</a>'
						}
						if (row) {
	...
	function addUser() {
		...
		$.ajax({
			type: "POST",
			url: ulrTo+"/azz/api/merchant/addMerchantUser",
			cache: false, //禁用缓存    
			dataType: "json", 
			data: {
				'merchantUserName': $("input[name='memberName']").val(),
	...
	//成员详情
	page.getUserInfo = function(merchantUserCode) {
		$.ajax({
			type: "GET",
			...
			success: function(data) {
				if (data.code == 0) {
					$("input[name='memberName2']").val(data.data.merchantUserName);
		...
		$('#myModal2').modal('show');
		$('#editconfirm').attr("onclick", "system.member.editUser(\'" + merchantUserCode + "\');")
	}
	
	//编辑成员
	page.editUser = function(merchantUserCode) {
		...
		$.ajax({
			type: "POST",
			url: ulrTo+"/azz/api/merchant/editMerchantUser",
			cache: false, //禁用缓存    
			dataType: "json", 
			data: {
				'merchantUserCode':merchantUserCode,
				'merchantUserName': $("input[name='memberName2']").val(),
	...
	//启用禁用
	page.editUserStatus = function(merchantUserCode,statustoo) {
		if(statustoo == '启用') {
			$.ajax({
				type: "POST",
				url: ulrTo+"/azz/api/merchant/editMerchantUserStatus",
				cache: false, //禁用缓存    
				dataType: "json", 
				data: {
					'merchantUserCode': merchantUserCode,
	...
	//删除成员
	page.delDeptInfo = function(merchantUserCode,merchantUserName) {
		$('#bmName').html(merchantUserName);
		$('#myModal112').modal('show');
		$('#deletebunnot').attr("onclick", "system.member.delDeptInfotoo(\'" + merchantUserCode + "\');")
	}
	page.delDeptInfotoo = function(merchantUserCode) {
		$.ajax({
			type: "POST",
			url: ulrTo+"/azz/api/merchant/editMerchantUserStatus",
			cache: false, //禁用缓存    
	...
	function importExcel(){
		...
		$.ajax({
	        type :'POST',
	        url : ulrTo + '/azz/api/merchant/importMerchantUser',
	        cache: false, //禁用缓存    
			dataType: "json",
